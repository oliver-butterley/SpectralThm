...
theorem toComplexMeasure_to_signedMeasure_neg (c : ComplexMeasure α) :
    SignedMeasure.toComplexMeasure (-ComplexMeasure.re c) (-ComplexMeasure.im c) = -c := rfl
...
theorem eq_iff (c c' : ComplexMeasure α) : c.re = c'.re ∧ c.im = c'.im ↔ c = c' := by
  constructor
  · intro h
    rw [← toComplexMeasure_to_signedMeasure c, ← toComplexMeasure_to_signedMeasure c', h.1, h.2]
